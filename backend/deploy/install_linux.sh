#!/usr/bin/env bash
# Instala Sortix como servicio de usuario systemd: arranca solo cuando
...
UNIT_FILE="$UNIT_DIR/sortix.service"
...
sed \
    ...
    "$BACKEND_DIR/deploy/sortix.service.template" > "$UNIT_FILE"

echo "==> Activando el servicio..."
systemctl --user daemon-reload
systemctl --user enable --now sortix.service

echo
echo "Sortix esta corriendo en segundo plano. Abre http://127.0.0.1:5000 para verlo."
echo "Estado:      systemctl --user status sortix.service"
echo "Logs:        journalctl --user -u sortix.service -f"
echo "Detenerlo:   systemctl --user stop sortix.service"
echo
